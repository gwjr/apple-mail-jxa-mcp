...
// Mail MCP Server using JXA MCP Framework
//
// Provides tools for interacting with Apple Mail via scripting.
...
const Mail = {
    ...
    messageFromUrl(url) {
        ...
        // 4. Full enumeration (TODO: could be expensive)
        return null;
    },
    ...
    (uri) => {
        ...
        if (match) {
            ...
            const mailboxMap = {
                inbox: () => app.inbox,
                drafts: () => app.drafts,
                sent: () => app.sentMailbox,
                junk: () => app.junkMailbox,
                trash: () => app.trash,
                outbox: () => app.outbox
            };
...
server.addTool({
    ...
    handler: (args) => {
        ...
        app.doShellScript(`mv ${$.NSString.alloc.initWithUTF8String(tempFile).js.replace(/'/g, "'\\''")} ${$.NSString.alloc.initWithUTF8String(destPath).js.replace(/'/g, "'\\''")}`);
